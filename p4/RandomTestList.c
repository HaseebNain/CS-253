    ...
    for (i=0; i<count; i++) {
        ...
        switch (test) {
            ...
            case 4:
				...
				WordObjPtr searchtmp = createWordObj("some info",1);	
                node = removeNode(list, search(list, searchtmp));
				freeNode(node, freeWordObj);
                break;
            case 5:
...
{	
	...
	list = createList(compareTo, toString, freeWordObj);
	for (i=0; i<n; i++)
	{
	  job = createWordObj("args",i);
		node = createNode(job);
		addAtFront(list, node);
	}
